...
import org.iesalizar.daw2.dominicarturoobilosorio.dwese_app_web.dtos.AuthRequestDTO;
import org.iesalizar.daw2.dominicarturoobilosorio.dwese_app_web.dtos.AuthResponseDTO;
...
public class AuthenticationController {
    ...
    /**
     * Punto de acceso para autenticar a los usuarios y generar un token JWT en caso de éxito.
     *
    ...
    public ResponseEntity<AuthResponseDTO> authenticate(@Valid @RequestBody AuthRequestDTO authRequest) {
        try {
            ...
            // 2. Obtiene el nombre de usuario autenticado
            String username = authentication.getName();

            // 3. Extrae los roles del usuario autenticado desde las autoridades asignadas
            List<String> roles = authentication.getAuthorities().stream()
                    .map(authority -> authority.getAuthority()) // Convierte cada autoridad en su representación de texto
                    .toList();

            // 4. Genera un token JWT para el usuario autenticado, incluyendo sus roles
            String token = jwtUtil.generateToken(username, roles);

            // 5. Retorna una respuesta con el token JWT y un mensaje de éxito
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new AuthResponseDTO(null, "Ocurrió un error inesperado. Por favor, inténtelo nuevamente."));
        }
    }
